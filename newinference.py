...
def inference_main(inference_args):
    ...
    if num_experts > 1:
        if inference_args.huggingface_checkpoint_folder is not None: # loading from huggingface checkpoint
            router = newControlNetRouter.from_pretrained(
                "hanlincs/Ctrl-Adapter", 
    ...
    elif inference_args.model_name == 'sdxl': ###### we use sdxl_controlnet_adapter_new_pipeline
        pretrained_model_name_or_path = "stabilityai/stable-diffusion-xl-base-1.0"
        from sdxl.pipelines.sdxl_controlnet_adapter_new_pipeline import SDXLControlNetAdapterPipeline
    ...
    for idx, sample in enumerate(samples): 
        ...
        if inference_args.model_name == 'i2vgenxl':
            ...
            print("control images len:", len(control_images))
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        ...
        if inference_args.eval_input_type == 'images':
            ...
            for i, ctrl_type in enumerate(inference_args.control_types):
                # it's not "directory + sample" as the directory, but the direct file path.
                cond_save_path = os.path.join(output_condition_images_dir[i], sample)
                # Ensure that the parent directory exists
                os.makedirs(os.path.dirname(cond_save_path), exist_ok=True)
                # Save a single condition image
                all_conditioning_images_pil[i][0].save(cond_save_path)
            # 3. save generated image
            output_images.save(os.path.join(output_images_dir, sample))
            # 4. save concatenated image
            generated_img = output_images  # PIL.Image
            target_size = generated_img.size  # (width, height)

            # Collect all condition diagrams
            cond_imgs = [
                all_conditioning_images_pil[i][0].resize(target_size, Image.Resampling.LANCZOS)
                for i in range(len(inference_args.control_types))
            ]

            # Splicing
            all_imgs = cond_imgs + [generated_img]
            total_width = target_size[0] * len(all_imgs)
            height = target_size[1]

            concat_img = Image.new('RGB', (total_width, height))
            for idx, img in enumerate(all_imgs):
                concat_img.paste(img, (idx * target_size[0], 0))

            concat_img.save(os.path.join(concat_output_dir, sample))
