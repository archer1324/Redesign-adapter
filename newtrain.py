...
from newinference import inference_main
...
def main(args):
    ...
    if num_experts > 1:
        assert model_name == 'sdxl', "please note that we only support i2vgenxl for multi-condition control in our current codebase"
        if args.router_type == 'timestep_weights':
            ...
            control_channels = [320, 320, 320, 320, 640, 640, 640, 1280, 1280, 1280, 1280, 1280] # 示例
            router = newControlNetRouter(
                num_experts = num_experts, 
                backbone_model_name = model_name, 
                router_type = args.router_type,
                control_channels_per_block=control_channels, 
                embedding_dim = None, 
                use_sparsemax=args.use_sparsemax
                )
        ...
        elif args.router_type == 'patch_mlp_embedding':
            control_channels = [320, 320, 320, 320, 640, 640, 640, 1280, 1280, 1280, 1280, 1280] # control channels for each controlnet block
            router = newControlNetRouter(
                num_experts=num_experts,               # the numbers of control nets
                router_type='patch_mlp_embedding',      # or 'patch_mlp_timestep', etc.
                control_channels_per_block=control_channels,
                mid_block_channel=1280,
                cond_dim=2048,                 # e.g., text embed dim if using cond embeddings
            )
    ...
    def collate_fn(examples: list) -> dict:
        ...
        if model_name in ['sdxl']:
            ...
            ######
            cond_emb = torch.concat([prompt_embeds_0, prompt_embeds_1], dim=-1).mean(dim=1)
    ...
    def compute_loss_from_batch(batch: dict):
        ...
        with torch.no_grad():
            ...
            if model_name in ['sdxl']:
                ...
                cond_emb = batch['cond_emb'].to(dtype=weight_dtype)
        ...
        ###### we change i2vgenxl to sdxl
        if num_experts > 1:
            assert model_name == 'sdxl',
            #with torch.no_grad():
            with torch.autocast(device_type="cuda", dtype=weight_dtype):
                if args.router_type == 'simple_weights' or args.router_type == 'equal_weights':
                    ...
                    down_block_weights, mid_block_weights = router(cn_feats_down = down_block_res_samples_dict, cn_feats_mid=mid_block_res_sample_dict,router_input=[prompt_embeds, timesteps], sparse_mask=sparse_mask)
        else:
        ...
        for k in range(num_cnet_output_blocks): ######
            for idx, ctrl_type in enumerate(current_batch_control_types):
                if down_block_weights[k].dim() == 4:  # [B, num_experts, H, W]
                    w = down_block_weights[k][:, idx, :, :].unsqueeze(1)  # [B, 1, H, W]
                    down_block_res_samples[k] += down_block_res_samples_dict[ctrl_type][k] * w
                else:
                    down_block_res_samples[k] += down_block_res_samples_dict[ctrl_type][k] * down_block_weights[k][0, idx]


        if 'M' in args.adapter_locations:
            for idx, ctrl_type in enumerate(current_batch_control_types):
                if mid_block_weights.dim() == 4:  # [B, num_experts, H, W]
                    w = mid_block_weights[:, idx, :, :].unsqueeze(1)  # [B, 1, H, W]
                    mid_block_res_sample += mid_block_res_sample_dict[ctrl_type] * w
                else:
                    mid_block_res_sample += mid_block_res_sample_dict[ctrl_type] * mid_block_weights[idx]
        ...
        if args.snr_gamma:
            ...
            if num_experts > 1 and down_block_weights is not None:
                log_down_blocks = [w.cpu().detach() for w in down_block_weights]
            else:
                log_down_blocks = None
    ...
    def process_batch(batch: dict):
        ...
        if num_experts > 1:
            for idx_r in range(num_cnet_output_blocks):
                block = down_block_weights[idx_r]  # [B, E, H, W]
                for idx_e, ctrl_type in enumerate(control_types):
                    logs[f'down_block_{idx_r}_{ctrl_type}'] = block[:, idx_e].mean().item()


                if mid_block_weights is not None:
                    for idx_e, w in enumerate(mid_block_weights):
                        ctrl_type = control_types[idx_e]
                        logs[f'mid_block_{ctrl_type}'] = w.mean().item()
