...
class ImageLoader(Dataset):
    def __init__(
        ...
        **kwargs
        ...
        for path in self.path:
            for ext in img_exts:
                self.image_files.extend(glob(os.path.join(path, ext)))


        # -------------------------------
        # 2. 读取所有 metadata（按文件名匹配）
        # -------------------------------
        if self.metadata_path is not None:
            for meta_path in self.metadata_path:
                if meta_path.endswith(".csv"):
                    df = pd.read_csv(meta_path)
                    df = df[['filename', 'caption']]
                    for _, row in df.iterrows():
                        fname = os.path.basename(row['filename'])
                        self.metadata[fname] = row['caption']
                    ...
                    for item in items:
                        ...
                        caption = item.get("llava_caption", item.get("caption", ""))
                        self.metadata[fname] = caption

        valid_files = []
        for img in self.image_files:
            if os.path.basename(img) in self.metadata:
                valid_files.append(img)

        self.image_files = sorted(valid_files)
    ...
    def __getitem__(self, index):
        for _ in range(10):  # 最多尝试十次，不无限循环
            ...
            try:
                ...
                if img_filename not in self.metadata:
                    print(f"[WARN] Metadata missing: {img_filename}")
                    index = random.randint(0, len(self.image_files) - 1)
                    continue
                ...
                pixel_values = image_to_tensor(image_pil)

                break

            except Exception as e:
                print(f"[ERROR] Loading failed: {img_path} | {e}")
                index = random.randint(0, len(self.image_files) - 1)

        else:
            raise RuntimeError("Too many image loading failures.")
