...
class ControlNetHelper(ModelMixin, ConfigMixin):
    ...
    def __init__(self, pretrained_model_name_or_path="runwayml/stable-diffusion-v1-5", use_size_512=True):
        ...
        # ensure attributes exist even if add_depth_estimator fails to set them
        self.estimator_ckpt_path = None
    ...
    def prepare_conditioning_images(self, batch_images_pils, current_batch_control_types, img_size=(512, 512), num_Frames=8):
        ...
        for control_type in current_batch_control_types:
            ...
            if control_type == 'depth':
                # ensure depth_estimator is initialized; try to use configured ckpt, otherwise fall back
                if getattr(self, 'depth_estimator', None) is None:
                    try:
                        # try to initialize with provided checkpoint path (may be None)
                        self.add_depth_estimator(getattr(self, 'estimator_ckpt_path', None))
                    except Exception as e:
                        print(f"Warning: failed to init MiDaS depth estimator with checkpoint {self.estimator_ckpt_path}: {e}")
                        try:
                            # fall back to transformers pipeline (slower)
                            self.add_depth_estimator(None)
                        except Exception as e2:
                            raise RuntimeError(f"Failed to initialize any depth estimator: {e2}")
